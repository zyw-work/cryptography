...
**答案：** ______6c73d5240a94878698ebc297804a_____
...
```
设 G: \{0,1\}^{\ell} \to \{0,1\}^n 是一个 PRG，A 是一个概率多项式时间（PPT）的区分器（统计测试）。A 攻击 G 的区分优势定义为：


Adv_{PRG}[A, G] = \left| \Pr_{s \leftarrow \{0,1\}^{\ell}} [A(G(s)) = 1] - \Pr_{r \leftarrow \{0,1\}^n} [A(r) = 1] \right|
...
```
接近1：区分器 A 能够极其有效地区分 G 的输出与真正的随机串
接近0：区分器 A 无法区分 G 的输出与真正的随机串。
```
...
```
不安全
理由： 密码学安全的伪随机生成器（PRG）的核心要求是：对于所有概率多项式时间（PPT）的区分器A，其优势Adv_{PRG}[A, G]必须是可忽略的
```
...
```
完美保密性（Perfect Secrecy）的定义是：对于任意明文 m \in \mathcal{M} 和任意密文 c \in \mathcal{C}，满足
\Pr_{k \leftarrow K}[E(k, m) = c] = \Pr_{k \leftarrow K}[E(k, m') = c]
对所有明文 m, m' \in \mathcal{M} 成立；
```
...
```
若 E(k, m) = c，则 m \oplus k = c，变形得 k = m \oplus c。
对于给定的 m 和 c，这个 k 是唯一确定的，因此恰好存在 \boldsymbol{1} 个密钥 k 使得 E(k, m) = c。
```
...
```
1. 由(b)可知：对任意明文 m, m' 和密文 c，满足 E(k, m)=c 的密钥数量 = 满足 E(k, m')=c 的密钥数量 = 1。

2. 密钥 k 是从密钥空间 K 中均匀随机选取的，因此
\Pr_{k \leftarrow K}[E(k, m) = c] = \frac{1}{|K|}
\Pr_{k \leftarrow K}[E(k, m') = c] = \frac{1}{|K|}
3. 因此 \Pr_{k \leftarrow K}[E(k, m) = c] = \Pr_{k \leftarrow K}[E(k, m') = c]，满足(a)中完美保密性的定义，故OTP具有完美保密性。
```
...
```
 因为存在高效算法 A 能从密文 c 中直接获取明文 m 的最低有效位，违反了语义安全“密文不泄露任何明文有效信息”的核心要求，故该加密方案不是语义安全的✅
...
```
应选择 \boldsymbol{m_0, m_1} 满足 \boldsymbol{\text{LSB}(m_0) \neq \text{LSB}(m_1)}（例如 m_0 最低位为0，m_1 最低位为1）；收到密文 c 后，调用 A(c) 得到结果 b'，直接猜测 b = b' 即可赢得游戏✅
...
```
因为密钥 k 均匀随机选取，对任意 c ∈{0,1}^n，Pr[k⊕ m_0 = c] = \frac{1}{2^n}，故密文 c 在 \{0,1\}^n 上均匀分布，Advss[B,E]=0
```
...
```
同 (a) 的证明逻辑，k 均匀随机，异或为置换，故 c=k\oplus m_1 均匀分布
```
...
```
EXP(0) 和 EXP(1) 的密文均为 {0,1\}^n 上的均匀分布，故分布相同。
```
